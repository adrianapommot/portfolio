...
- **Cache-busting:** ao mudar QUALQUER `.css`/`.js`, subir o `?v=N` em TODAS as páginas que carregam o arquivo. Versões atuais: `home.css?v=5`, `site-nav.js?v=16`, `case-patterns.css?v=16`. Sempre incrementar acima do que está no ar — nunca reverter.
- **Origem das mudanças de design:** o trabalho de design é feito no projeto do Claude (claude.ai) e chega ao repo via export. O Claude design só LÊ o repo (não edita); quem comita os exports é o deploy. O Claude design espelha o `main` antes de exportar para não regredir CV nem versões de cache. O repo é a fonte da verdade do que está no ar, e por isso é o ponto onde travessões/refs quebradas são barrados (ver guard abaixo).
- **Fluxo de deploy (2 passos):** depois de copiar os arquivos do export para a raiz, SEMPRE rodar **`bash tools/deploy-sanitize.sh`** ANTES do commit. Ele (1) corrige os travessões conhecidos que o design re-exporta toda rodada (copy/alt = vírgula/dois-pontos/ponto; title do Delta = ponto-médio ·) e (2) trava o `?v=` de home.css/case-patterns.css: nunca abaixo do que está no ar, +1 automático quando o conteúdo do CSS mudou (o export costuma regredir case-patterns para v14). No fim ele chama o guard; se sobrar travessão NOVO/desconhecido, corrigir à mão e rodar de novo. Depois é só `git add -u` + commit. Substituições de travessão são em perl UTF-8 (`-CSD -Mutf8`).
- **Guard de deploy (anti-regressão):** hook de pré-commit em `.githooks/pre-commit` (ativado por `git config core.hooksPath .githooks`) que roda `tools/deploy-check.sh`. BLOQUEIA o commit se houver travessão (— ou –) em texto renderizado (texto, alt, title, aria-label, placeholder) ou referência de imagem quebrada; AVISA sobre raster >400KB fora de webp. É a rede de segurança caso o sanitize não tenha sido rodado. Rodar à mão: `bash tools/deploy-check.sh`. Minus (−), setas e ponto-médio (·) são permitidos (não são dash).
